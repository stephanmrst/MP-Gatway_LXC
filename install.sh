...
# Interaktive Eingaben immer direkt vom Terminal lesen. Das funktioniert
# zuverlässig auch beim Aufruf über: bash <(curl -fsSL URL)
if [[ -r /dev/tty ]]; then
  exec 3</dev/tty
else
  exec 3<&0
...
printf '\n== MP-Gateway Proxmox-LXC-Installer 0.5 ==\n'
